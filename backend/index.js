...
const supabase = createClient('https://abgspujwyujtccknqenr.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFiZ3NwdWp3eXVqdGNja25xZW5yIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTA0MzU5NzgsImV4cCI6MjAyNjAxMTk3OH0.NOjPxUVPBYztUlLCl6CBYg9vIrl9I58zD6bolUzqYfs');
...
const conexion = async(id,currentFirstName,currentSecondName)=>{
  ...
  try {
    ...
    const insertGroup = supabase
      .from("savegroups2")
      .insert([{ id, currentGroupName: currentFirstName, currentSecondGroup: currentSecondName }]);
    ...
    const [groupResult, messageResult] = await Promise.all([insertGroup, insertMessage]);
    ...
    if (messageResult.error) {
      ...
      console.log("Datos insertados correctamente en messages:", messageResult.data);
    }
  } catch (error) {
...
async function loadChatGroups() {
  try {
    const { data: groupsData, error: groupsError } = await supabase.from("savegroups2").select("*");
...
socketIO.on("connection", (socket) => {
  ...
  socket.on("createNewGroup", async({ currentGroupName, currentSecondGroup }) => {
    try{
...
socket.on("findGroup", (id) => {
  ...
  socket.on("newChatMessage", async(data) => {
    ...
    // await sendMessages(groupIdentifier,newMessage)
      ...
      const{data,error} = await supabase.from("messages").update([{"messages":filteredGroup[0].messages}]).eq("idGroup",groupIdentifier)
      if (error){
        console.log(error)
      }
      else{
