...
import { View, Text, Image, TouchableOpacity, Alert, SafeAreaView, TextInput, Modal, ScrollView } from "react-native";
import { Camera, MapPin, Mail, User, Droplet, MessageSquare, Edit2, Heart, UserPlus, Phone } from "react-native-feather";
...
const Customer = ({}) => {
  ...
  const pickImage = async () => {
    ...
    try {
      ...
      if (!result.cancelled) {
        ...
        const { data: data2, error: error2 } = await supabase.storage.from("prueba").getPublicUrl(fileName)
        ...
        const { data, error } = await supabase.from("users").update({ Url: imageUrl }).eq("Email", email)
  ...
  const UserProfileItem = ({Icon, title, content, isEditable, editFunction}) => {
    return (
      <View className="px-4 py-3 bg-white mb-2 shadow-sm">
        <View className="flex-row items-center justify-between mb-2">
          <View className="flex-row items-center">
            <Icon stroke="#FF4136" width={20} height={20} className="mr-2" />
            <Text className="text-lg font-bold text-gray-800">{t(title)}</Text>
          </View>
          {isEditable &&
  ...
  const handleLocationUpdate = (newState, newMunicipality) => {
    setUser((prev) => ({ ...prev, State: newState, City: newMunicipality }));
  };
  ...
  return (
    <View className="flex-1">
      <ScrollView className="flex-1 bg-gray-100">
        ...
        <View className="items-center p-6 pt-0 bg-red-500 flex flex-row gap-3">
          ...
          <View>
            ...
            {isLoading ? "..." : (`${user?.FirstName.length > 17 ? user?.FirstName.slice(0, 15) + "..." : user?.FirstName}`)}
            </Text>
            <Text className="text-2xl font-bold text-white">
              {isLoading ? "..." : (`${user?.LastName.length > 17 ? user?.LastName.slice(0, 15) + "..." : user?.LastName}`)}
            </Text>
          </View>
        </View>
        ...
        <UserProfileItem
          Icon={MapPin}
          title={t("location")}
          content={isLoading ? "..." : `${user?.City}, ${user?.State}`}
          isEditable={true}
        ...
        <UserProfileItem
          Icon={User}
          title={t("gender")}
          content={isLoading ? "..." : user?.Gender === "male" ? t("male") : t("female")}
          isEditable={false}
        />
      ...
      <ModalUpdatePhone
        ...
        onPhoneUpdated={(newPhone) => setUser((prev) => ({ ...prev, Phone: newPhone }))}
        setIsLoading={setIsLoading}
      />
